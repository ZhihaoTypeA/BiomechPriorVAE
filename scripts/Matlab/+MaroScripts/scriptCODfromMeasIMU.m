...
function scriptCODfromMeasIMU(path, pID, trialType, settingsInpt) 
    ...
    if useVAE
        ...
        vaeParams.modelPath  = fullfile(vaeFilePath, 'BiomechPriorVAE_best.pth');
        vaeParams.scalerPath = fullfile(vaeFilePath, 'scaler.pkl');
        vaeParams.pythonPath = vaeFilePath;

        fprintf('VAE File Path: %s\n', vaeFilePath);
